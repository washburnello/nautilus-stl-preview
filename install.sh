...
# - grid thumbnails (.thumbnailer) -> ~/.local/share/thumbnailers/
#   (requires the `stl-thumb` binary, e.g. `yay -S stl-thumb`)
set -euo pipefail
cd "$(dirname "$0")"
...
if command -v stl-thumb >/dev/null 2>&1; then
  cp thumbnailers/stl-thumb.thumbnailer "$THUMB_DIR/stl-thumb.thumbnailer"
  echo "installed thumbnailer (stl-thumb found)"
else
  echo "NOTE: stl-thumb not found — skipping thumbnailer."
  echo "      install it (e.g. yay -S stl-thumb) and re-run install.sh"
fi
